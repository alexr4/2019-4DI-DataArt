...
void setup() {
  ...
  videoExport = new VideoExport(this, "SolineLoopedNoise4.mp4");
...
void draw() {
  ...
  for (int i = 0; i<arbres.size(); i++) {
    ...
    /* 
     ...
     72.6°, 72.4, 82.4
     */
    float hue =  map(numberOfTrees, minmax[0], minmax[1], 0, 200);
    float sat =  map(numberOfTrees, minmax[0], minmax[1], 0, 30);
    float brigh =  map(numberOfTrees, minmax[0], minmax[1], 100, 30 );
